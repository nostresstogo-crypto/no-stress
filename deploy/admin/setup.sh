#!/usr/bin/env bash
# ────────────────────────────────────────────────────────────────────────────
#  One-shot bootstrap for nostress-admin on the VPS.
#
#  Run as root on the VPS, ONCE:
#      sudo bash deploy/admin/setup.sh
#
#  - Creates /var/www/nostress-admin/{releases,current}
#  - Installs nginx vhost for admin.no-stress.net (HTTPS, with /api proxy
#    to the local nostress-api service on 127.0.0.1:8080).
#  - Disables any pre-existing admin.no-stress.net vhost (backed up to .OLD-BACKUP).
#  - Assumes Let's Encrypt cert already exists at
#    /etc/letsencrypt/live/admin.no-stress.net/.
# ────────────────────────────────────────────────────────────────────────────
set -euo pipefail

DOMAIN="admin.no-stress.net"
APP_USER="nostress"
...
# Managed by deploy/admin/setup.sh — do not edit manually.
server {
    listen 80;
    listen [::]:80;
    server_name ${DOMAIN};
...
cat > /etc/sudoers.d/nostress-admin <<SUDOERS
...
SUDOERS
chmod 0440 /etc/sudoers.d/nostress-admin
visudo -c -f /etc/sudoers.d/nostress-admin
...
echo ""
echo "👉 Next: push to main — the GitHub Actions workflow will deploy the first release."
