...
import TabsIndexService from "../services/tabs.index.service";
...
function CityPickerModal({
  ...
  const t = useT();
  const insets = useSafeAreaInsets();
  const items = [{ _id: "", name: t('allcities') }, ...cities];
      ...
      <View style={[modal.sheet, { paddingBottom: insets.bottom + 16, backgroundColor: C.card }]}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity
                style={[modal.item, active && { backgroundColor: C.lavender + "18" }, 
                    { borderBottomColor: C.border }]}
                onPress={() => { onSelect(item._id); onClose(); }}
              >
...
function TypePickerModal({
  ...
  const t = useT();
  const insets = useSafeAreaInsets();
  const items = [{ _id: "", name: t('alltypes') }, ...types];
...
export default function MapScreen() {
  ...
  const tabsIndexService = new TabsIndexService();
  const C = useColors();
  ...
  const getPlaceTypes = () => {
        ...
        venueService.getPlaceTypes().then(async function (response: any) {
            console.log(response);
            setPlaceTypes(response.data?.data.data);
    ...
    const getPlaces = () => {
        setPlacesLoading(true);
          ...
          venueService.getPlaces(selectedType).then(async function (response: any) {
            console.log(response);
    ...
    const getCities = () => {
        ...
        tabsIndexService.getCities().then(async function (response: any) {
            console.log(response);
  ...
  useEffect(() => {
    ...
    //     const list: Place[] = (Array.isArray(data?.venues) ? data.venues : [])
    //       .map((v: any) => ({
    //         id: `api_${v.id}`,
    //         name: v.name || "",
    //         type: v.type || "",
  ...
  /* ─── Apply distance filter on `places` (only when geolocation is available) ─── */
  const filteredPlaces = useMemo<Place[]>(() => {
    if (maxDistanceKm == null || !userLocation) return places;
    return places.filter((v: Place) => {
  ...
  }, [places, maxDistanceKm, userLocation]);

  /* ─── Compute distance + sort nearest-first when location available ─── */
  const venuesWithDistance = useMemo(() => {
    if (!userLocation) {
      return filteredPlaces.map((v: Place) => ({ venue: v, distanceKm: null as number | null }));
    }
      ...
      .map((v: Place) => ({
        ...
        distanceKm:
          v.lat != null && v.lng != null
            ? haversineKm(userLocation.lat, userLocation.lng, Number(v.lat), Number(v.lng))
            : null,
      }))
      .sort((a, b) => {
  ...
  const htmlContent = useMemo(() => {
    return buildLeafletHtml(filteredPlaces, selectedVenue?._id ?? "", userLocation);
  ...
  const handleMessage = useCallback((data: any) => {
    ...
    if (parsed?.type === "venueSelected") {
      const v = places.find((x: Place) => x._id === parsed.venueId);
      if (v) setSelectedVenue(v);
    }
  }, [places]);

  const flyTo = (venue: Place) => {
    setSelectedVenue(venue);
    mapRef.current?.postMessage({ type: "flyToVenue", venueId: venue._id });
  };
...
//   }, [selectedCity]);
    ...
    const selectedCityLabel = useMemo(() => {
      if (selectedCity) {
        const cityObj: any = cities.find((c: any) => c._id === selectedCity);
    ...
    const selectedTypeLabel = useMemo(() => {
      if (selectedCity) {
        const tObj: any = placeTypes.find((t: any) => t._id === selectedType);
        return tObj?.name ?? selectedType;
      }
      return null;
    }, [selectedType]);
  ...
  return (
    <View style={[styles.root, { backgroundColor: C.bg }]}>
      ...
      <View style={[styles.header, { paddingTop: topInset + 10, backgroundColor: C.bg, borderBottomColor: C.border }]}>
        <View style={styles.headerRow}>
          <View>
            ...
            <Text style={[styles.headerSub, { color: C.textMuted }]}>
              ...
              {lang === "fr"
                ? filteredPlaces.length > 1 ? "lieux" : "lieu"
                : filteredPlaces.length > 1 ? "venues" : "venue"}
        ...
        {/* Filter row: Villes + Distance + type pills */}
      ...
      {/* Type picker modal */}
      {
        !placeTypesLoading && <TypePickerModal
      ...
      <View style={styles.mapContainer}>
        <MapWebView
          ref={mapRef}
          key={`${mapKey}-${filteredPlaces.map((v: Place) => v._id).join(",")}-${userLocation ? "u" : "n"}`}
        ...
        {selectedVenue !== null &&  (
          <View style={[styles.venueCard, { backgroundColor: C.card, borderColor: C.border }]}>
            ...
            <View style={styles.venueCardContent}>
              ...
              <View style={styles.venueCardInfo}>
                ...
                <View style={[styles.venueTypePill, { backgroundColor: C.lavender + "18", borderColor: C.lavender + "40" }]}>
                  <Text style={[styles.venueType, { color: C.lavender }]}>{selectedVenue.type?.name}</Text>
                </View>
                <View style={styles.venueAddressRow}>
                  <Ionicons name="location-outline" size={11} color={C.textMuted} />
                  <Text style={[styles.venueAddress, { color: C.textMuted }]} numberOfLines={1}>
                    {selectedVenue.city?.name} · {selectedVenue.address}
                  </Text>
                </View>
              </View>
            </View>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                router.push(`/partner/${selectedVenue._id}`as any);
                setSelectedVenue(null);
              }}
              activeOpacity={0.85}
            >
...
type VenueWithDistance = { venue: Place; distanceKm: number | null };
      ...
      {open && (
        <ScrollView style={list.scroll} showsVerticalScrollIndicator={false}>
          {items.map(({ venue: v, distanceKm }) => {
            const active = selectedVenue?._id === v._id;
            return (
              <TouchableOpacity
                key={v._id}
